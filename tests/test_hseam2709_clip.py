"""The NOVEL hseam2709 corner S=(-1744.1967, 752.9029) (interior 107.43, walls 2709 x 2919) -- delivered
...
`_generated/novel_hseam2709.json`.
"""
import json
import os
import struct
...
_GOLD = os.path.join(os.path.dirname(_HERE), 'fixtures', 'hseam2709_rest_golden.json')
...
pytestmark = pytest.mark.skipif(not _HAVE, reason="rollstab harness / hseam2709 golden unavailable")

ANCHOR = 'hyrule_rollstab_hseam2709@twwgz'
...
def test_hseam2709_rest_bitexact():
    ...
    assert not bad, "hseam2709 from-rest diverged at rows %s" % bad


_SHIP = os.path.join(os.path.dirname(_HERE), 'fixtures', 'hseam2709_roll_ship_golden.json')


@pytest.mark.xfail(condition=not os.path.exists(_SHIP), strict=True,
                   reason="hseam2709 clip UNDELIVERED: honest 97m-class lottery (band_dense "
                          "0.011-0.014u; 8 knob-family draws, 0 genuine) -- goes GREEN when a "
                          "draw hits and `deliver ship` writes the golden (session 62)")
def test_hseam2709_clip_delivered():
    """The hseam2709-corner roll-stab clip, found by the generalized `solver.solve_focused` and
