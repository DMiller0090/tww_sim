...
- **My search calls its MOST-overlapping candidate its closest - is deeper contact nearer a clip? Where in overlap does a real clip sit / how much of a billion scorings could ever have clipped / can I prefilter the overlap?** → [strategy/clip-overlap-band.md](strategy/clip-overlap-band.md)
...
- **My herd ends mid-BACKSLIDE and the walk burns frames converting - is that the herd's nature or an input? Do I need a camera slew to start the walk at cap / does converting move Tetra / why does my at-cap walk still miss every station at short walks?** → [strategy/cap-entry-conversion.md](strategy/cap-entry-conversion.md)
- **My fan hit swept genuine but `confirm_entry` refused it with every walk flag green and `entry` off by 5-8 u - what moved? Is a "0 genuine" trustworthy when the walk ends NEAR Tetra / whose pose does the roll-dispatch frame's CC use / why did the console rediscovery never catch this?** → [mechanics/entry-frame-recoil.md](mechanics/entry-frame-recoil.md)
- **Why did a confirm-ALL-GREEN plan's A press start a CONVERSATION on console instead of the roll (proc 170) / which state decides talk-vs-roll / what exact bearing does the +-90 deg talk cone compare?** → [mechanics/talk-eat.md](mechanics/talk-eat.md)
...
- **WHICH body lean do I build a roll's razor at - the walk's `m351C`, the herd's, or something else? Why did a terminal that solved genuine stop clipping when I re-measured it / is the lean a constant of the corner or a knob / how do I stop a native fast path handing me a stale field as a measurement?** → [strategy/the-lean-is-the-rolls-own-dispatch.md](strategy/the-lean-is-the-rolls-own-dispatch.md)
...
- **My razor search sweeps thousands of roll ENTRIES and the residual barely moves - is the entry doing anything? Can I prefilter "out of contact at the cut" / why is nearly every row of a fan identical / if the entry is inert, what is left to aim at, and how accurately?** → [model/braced-cut-frame.md](model/braced-cut-frame.md)
- **My sweep reports `|resid| = 3e-06` beside 0 genuine - how is a row ON the razor refused? What should a search MINIMISE if not the residual / why does one f32 ULP either side of a clipping row fail / is `resid` a proxy for a clip or the thing itself?** → [model/genuine-residual-band.md](model/genuine-residual-band.md)
- **My item came back barren after an hour of fanning - was the search too small, or can that configuration not clip at all? Which axis do I steer (entry / lean / where she stands), how wide is each window, and can I check before paying for a fan?** → [model/admitting-configurations.md](model/admitting-configurations.md)
- **My screen says "no clip for ANY position of hers" - how much of her plane did that cover? Why does a Newton onto the residual never converge / why is there no gradient to descend from a seed a few u away / how far apart may my stations be?** → [model/razor-zero-curve.md](model/razor-zero-curve.md)
- **How fine does a LEAN sweep have to be before its zeros are real / how many distinct roll leans are there / can a planner steer the lean at all?** → [model/lean-cells.md](model/lean-cells.md)
- **WHERE can this corner clip at all, in coordinates a walk can aim at? Which axis separates a delivered clip from a barren item on the same cell and thrust / how far must I move the roll entry before a barren configuration clips?** → [model/admitting-entry-region.md](model/admitting-entry-region.md)
- **How far is my barren row from clipping, in u a WALK can steer? Is the roll entry an aimable axis or only where she stands / how accurately must a plan place it / can I SOLVE for an entry that clips instead of drawing for one / may I re-use a band measured elsewhere?** → [model/entry-strip.md](model/entry-strip.md)
- **My search reports 0 genuine over hundreds of items - is the space empty or can my GENERATOR not express the answer? What does a containment gate have to check / which barren results survive finding out / what does containing the known answer cost?** → [model/fan-containment-gap.md](model/fan-containment-gap.md)
- **The run found 8 genuine where the arithmetic said hundreds - which factor lied? Why did a row \|resid\| 3.7e-07 refuse / can I know an item's yield without paying its ~7 h / how do I rank the re-run queue by something better than its floor?** → [model/admitting-draws.md](model/admitting-draws.md)
- **I know where my plan's walk has to END - can I skip the branches that cannot get there? Why can't I search the held stick coarse-to-fine / why does knowing the endpoint barely narrow the enumeration?** → [model/aiming-the-fan.md](model/aiming-the-fan.md)
...
- **My clip needs a razor-thin overlap on one frame but my push produces whatever it produces - can a PUSHING roll ever arrive at that depth? Do I have to place the pushed actor precisely before the last roll / can the last push roll BE the clip roll or must the pusher back off and come in clean / does the body lean rule approaches out?** → [strategy/the-corner-sets-the-depth-not-the-herd.md](strategy/the-corner-sets-the-depth-not-the-herd.md)
- **I solved which PAIR clips - how do I ask whether my herd can deliver one? My terminal solve pinned the pusher on a line my herd never arrives on, is it still usable / is the ask to place the pushed actor or the pusher / why does a coordinate that reads exact turn a genuine cell dead?** → [strategy/the-razor-is-on-the-pusher-not-the-pushed.md](strategy/the-razor-is-on-the-pusher-not-the-pushed.md)
- **My search screens a whole FAN of variants off one node and each costs a full rollout - is any of that work shared? Is the camera an input to a roll or only an output / what does a roll segment actually contain / how do I port a hot stage without changing which candidates survive it?** → [strategy/the-fan-pays-for-one-camera.md](strategy/the-fan-pays-for-one-camera.md)
- **My herd's last cycle lands nowhere near the terminal I solved - is that a bad rank or something structural? What must the cycle BEFORE it hand over, and is that requirement a constant of the problem or does it move when I change the terminal / can I move the entry band by cutting the roll earlier / how do I make a razor-scale predicate cheap enough to rank a beam on?** → [strategy/the-crossing-and-the-runway-are-one-resource.md](strategy/the-crossing-and-the-runway-are-one-resource.md)
- **My search stage fires hundreds of rollouts and keeps three - what does it actually need from each one? How do I drop a fast kernel into a stage without changing which candidates survive it / why did my port land far below the ratio that named it / how do I pick a seed for a gate that compares two implementations?** → [strategy/a-screen-needs-a-record-not-a-run.md](strategy/a-screen-needs-a-record-not-a-run.md)
- **My stage re-runs the same rollout under N variants of ONE input - how much of that is shared, and where does the divergent tail start? Should I reach for the fast kernel that worked one stage up / why can a ported endpoint be exact and still be unusable by the next stage / how do I tell a gate that passed from a gate that was vacuous?** → [strategy/the-shared-roll-body.md](strategy/the-shared-roll-body.md)
- **My beam expands a node by a whole input ALPHABET and steps the frame once per child - do the children differ at all? I ported the hot stage and the search barely moved, what did I mis-measure / does a one-frame probe need a camera / when should I sweep a flag's defaults?** → [strategy/the-frame-the-alphabet-shares.md](strategy/the-frame-the-alphabet-shares.md)
- **My search keeps landing short of a target it should reach - is that reachability or a CUT? Why do the endpoints that get closest all violate the constraints my keeps rank on / what happens to a keep stack when the objective changes shape under it / where is the cheapest place to ask a new question?** → [strategy/the-axis-the-endgame-is-denominated-in.md](strategy/the-axis-the-endgame-is-denominated-in.md)
- **I freed the constraint my search was dying on and it STILL returns nothing - did I free the wrong one? What decides whether a post-roll endpoint can turn out of the talk cone in time / why do the endpoints that reach my target all fail the next stage?** → [strategy/the-crossing-costs-the-arming-posture.md](strategy/the-crossing-costs-the-arming-posture.md)
- **My screen reports its widest survivor sitting on the window edge - is the window clipping anything or is that the tail? How wide should a per-aim fan be / I sampled two parents to size a knob cheaply and got a clean answer, is it the right one?** → [strategy/the-window-binds-on-the-parents-that-produce.md](strategy/the-window-binds-on-the-parents-that-produce.md)
- **My search's biggest death counter is byte-identical under every knob I try - is that the wall? How do I tell a refusal that BINDS from one that is arithmetic / is my threshold clipping a reachable tail and what does relaxing it cost / why can a huge counter be worth nothing to attack?** → [strategy/the-biggest-death-counter-was-the-alphabet.md](strategy/the-biggest-death-counter-was-the-alphabet.md)
- **My search named its binding cut and I want to build it a better rank - what do I measure first? Can a cut be load-bearing and still hide nothing / how do I perturb a selection without inventing a new criterion / when does a null retire a whole TERM of the bound?** → [strategy/price-a-cut-in-both-directions.md](strategy/price-a-cut-in-both-directions.md)
- **Can a beam hold the same coupled state twice under different input logs? What free check bounds a cut-pricing run before it starts / why did a 16-of-31 beam cut price 0.00 in BOTH directions / what does a cut over twins actually select?** → [strategy/count-the-states-before-pricing-a-cut.md](strategy/count-the-states-before-pricing-a-cut.md)
- **My cut keeps 6 of 259 - how many distinct STATES do those 6 slots reach? My population is 1266 members and 34 states, which is the census unit / why does the same keep carry a per-state cap at one cut and none at the next / what does a population-complete widen actually cost?** → [strategy/a-keeps-width-is-not-its-reach.md](strategy/a-keeps-width-is-not-its-reach.md)
- **My beam ranks by distance to a solved target -- what if the target was never CONFIRMED to exist? Every candidate scored well and confirmed zero / how do I tell under-sampling from absent geometry / where is the Courtyard genuine Tetra set and why is the herd missing it?** → [strategy/confirm-the-terminal-before-you-rank.md](strategy/confirm-the-terminal-before-you-rank.md)
- **My cut keeps 400 of 5616 - do I raise the cap or re-compose the slots I already pay for? Which widen direction is free / how do I widen without losing the reproduction guard / why did the direction reaching 7x more states produce FEWER new identities?** → [strategy/a-cut-widens-two-ways.md](strategy/a-cut-widens-two-ways.md)
...
- **My roll fires its cut at the cheapest thrust in the window -- why does nothing CLIP there? Which Courtyard thrusts actually reach the seam and what does the cheapest one cost / how do I tell absent geometry from a thin scan when a scan returns zero / does the body lean move the terminal family if it is spent before the cut?** → [strategy/dispatchable-is-not-clipping.md](strategy/dispatchable-is-not-clipping.md)
- **My terminal scan gives a box in `(runway, along, lat)` -- can I check a delivered state against it by PROJECTING? Why did "4 of 49 rungs satisfy it" become 0 on re-measurement / when does a rank have to become a keep / my screen refuses the very hits its window was read from, what is wrong with a grid-sampled extent?** → [strategy/re-point-the-handoff-dont-re-project-it.md](strategy/re-point-the-handoff-dont-re-project-it.md)
- **My delivered states miss the terminal box by 30 u -- is that the whole miss? Why did screening on that box refuse a whole population for the wrong reason / which handoff coordinate belongs to the PUSHED actor and which can an aim still buy / my family fixture has no `side` column, what is it silently asserting?** → [strategy/the-box-cannot-see-the-lateral.md](strategy/the-box-cannot-see-the-lateral.md)
- **My sim warns it stops being faithful past a threshold and my search maximises over that population anyway -- what is the maximum worth? How do I audit a banked bound for domain membership without re-running its census / what buys the same quantity INSIDE the domain?** → [strategy/a-bound-read-past-the-guard-is-not-a-bound.md](strategy/a-bound-read-past-the-guard-is-not-a-bound.md)
...
- **My fast engine REFUSES to carry one model and I have queued a port behind the value it needs - is that a real gap or a guard? The value is constant in my regime, so why export it / why did moving my search to the fast engine silently stop my state pokes working / what is left of a stage once it takes zero wired steps?** → [model/the-camera-on-the-native-frame.md](model/the-camera-on-the-native-frame.md)
- **My fast engine is MISSING a branch the slow one has and it does not complain - what is that costing me, and how do I stop the gap lying to me? Where does a C step get collision / keyframe data from / what must a 0-ULP gate compare for a ported branch / why port a collision pass instead of pruning around it?** → [model/the-branch-a-fast-engine-skips.md](model/the-branch-a-fast-engine-skips.md)
