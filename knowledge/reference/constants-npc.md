...
**Source:** per-row. Split from [constants.md](constants.md) (the page hit its size cap,
2026-07-23); same contract - this table is the single source of truth, other pages link here
...
All live-confirmed on GZLJ01 (2026-07-06).
...
live-confirmed at the courtyard f0 (`fixtures/courtyard_zl1look.json`).
...
| Chase step | **0x1000** while `field_0x7BC < 0` (the courtyard case); else **0x0180** | `cLib_addCalcAngleL(angle, tgt, 4, step, 4)` all four angles | HIO `mMaxTurnStep` / `field_5C`; live f0 `f7bc = -1` |
