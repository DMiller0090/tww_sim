...
(dead-end #55; fix = `mint.draw_base` (CLI `drawbase=`), a one-time B-press draw + steady-WAITS
re-probe saved as a `_drawn` base, the #47/#49 setup pattern). A grazing `aim_deg` can trade dust density for a flat
