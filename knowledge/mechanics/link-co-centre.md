...
how? Two ports compute it and disagree by a ULP - which one do I delete?
**Status:** validated live. The midpoint port is bit-exact vs the game's `mCyl` on a pinned roll
(`fixtures/hyrule_roll_lean.json`), the **two-lean** form is bit-exact vs the courtyard console across
a whole clip roll at leans an order of magnitude larger (`fixtures/courtyard_clip_s86_console.json`),
and the 1-2 ULP that the two ports below used to disagree by was settled on console 2026-08-02 - it was
an anim-frame ULP, not a centre (see
[the two ports](#the-two-ports-and-what-was-actually-between-them)). The push that consumes it is
...
## The two ports, and what was actually between them
...
They used to part company by 1-2 ULP on isolated roll frames, which at the razor is the entire
verdict. **Neither was wrong.** They were sampling `rollf` at two different f32 FRAMES: the pose
driver's frame ctrl held `enter_roll`'s Python double `1.1` where `J3DFrameCtrl::mRate` is f32, and at
roll frame 2.2 -> 3.3 the true f32 sum is an exact tie the double falls short of - so it rounded down
where `LandState.roll_frame`, accumulating from an f32 `ROLL_RATE`, rounded half-to-even up. One ULP of
anim frame, 3 ULP of root translate, 1 ULP of this midpoint, and the clip verdict with it. The rule and
the arithmetic are [model/anim-frame-is-f32.md](../model/anim-frame-is-f32.md); with `FrameCtrl` f32 at
its own boundary the two ports agree bit-for-bit.

**A quantity computed two ways needs a gate that compares the two.** Each port had its own gate and
neither could see this - `tests/test_body_co_native.py` compares FootFK's native fold against FootFK's
own Python loop. `tests/test_centre_seam.py` is the one that compares the ports, and it also pins the
console run that settled which was right: a **blocked** candidate, where the two predicted 49.8582 u
and 0.1534 u - 49.97 u apart, so one run could not come back ambiguous
(`fixtures/courtyard_centre_seam_s90_console.json`, immutable). The dated narrative of the seam while
it was open is [history/co-centre-two-ports.md](../history/co-centre-two-ports.md).
