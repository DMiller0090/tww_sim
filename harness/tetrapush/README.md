...
- [~] **Build the planner** -- STARTED (session 28); the forward model is now FULLY BIT-EXACT from
      ...
      - [~] **THE OBJECTIVE IS NOW THE SEARCH'S RANK AND PRUNE, AND RUNNING IT PUT THE BLOCKER ON THE
            ...
            - **THE CHAIN, RE-MEASURED ON TODAY'S MODEL (piece 3): cycle 1 275.8 u / 21 f / 13.135 u/f
              (bound 71.9), cycle 2 590.9 u / 46 f / 12.846 (bound 72.9), CYCLE 3 STALLS -- zero roll
              survivors.** s43's 868.3 u / 69 f / 3 cycles does not reproduce (every s42-s51 number
              predates the s55-s59 model fixes, as the s60 box warned). At cycle 3 `junction_beam`
              still yields ~1990 gate-passing endpoints but `roll_probe` finds **996 of 1000 probed
              UNROLLABLE**, and the 4 survivors then die in `roll_candidates`. **And the lateral is
              already blown one cycle earlier**: cycle 1 leaves Tetra at lat +5.8 (inside the window,
              near the thread's +7.9) and cycle 2 swings her to **-39.9**, `lat_error` **-47.8 u**,
              ~5x the window -- so even a working cycle 3 could not place her.
            - **AND THE SEARCH'S CYCLE-2 BEAM DOES NOT CONTAIN THE HUMAN'S OWN CYCLE 2** -- which is
              the `[[search-space-contains-human]]` failure signature, and the sharpest statement of
              what is wrong. Scored side by side (`objective score`): the recorded 44-frame window ends
              at **lat +5.47, `lat_error` -2.48, `placeable` TRUE, bound 72.9**; the search's 46-frame
              2-cycle chain ends at **lat -39.9, `lat_error` -47.8, `placeable` FALSE, bound 72.9**.
              The bound calls them EQUAL because the search buys 30 u of extra along for 45 u of
              lateral -- and pd treats those as interchangeable when they are not: along is free to
              trade later (0.216 u of lateral per u of along), while correcting lateral needs Link to
              reposition to her SIDE and costs frames. So an on-line 2-cycle chain provably exists
              (his) and every one of the 7 beam survivors veers off it; **not one node stays inside the
              window** (lat -39.9 .. -54.0). The surviving roll aims are only **-4.5 deg / +5.0 deg**
              off the herd bearing, so this is not an off-axis aim -- it is the pursuit geometry: the
              box tolerates Link **+-18 u** off-line, and 18 u of offset ejects Tetra ~3.9 u/frame
              laterally (~97 u per cycle; even 4 u of offset gives ~22 u), so the lateral is set
              per-frame by where Link stands, not by the roll's heading.
            ...
            - **NEXT: the chain, and the lateral first.** (1) **Re-run the cycle-2 stage from the
              human's OWN f21 exit** and read the lateral of what it finds -- s42's own controlled
              experiment, and now it has a sharp question: if his entry state also yields only veering
              survivors, the GATES drop the on-line branch; if it stays on-line, the blocker is cycle
              1's exit state. Either answer names the fix. (2) Control the lateral where it is
              actually set -- per frame, by Link's own offset (the box's +-18 u is ~5 windows' worth of
              drift per cycle) -- rather than by re-ranking cycle endpoints. Consider ranking Tetra's
              `lat_error` explicitly, or `h = max(along_remaining, lateral_remaining / measured
              lateral rate)` so the bound stops treating a lateral miss as cheap as an along one.
              (3) Cycle 3's stall -- read the new `aim_*` counters, do not guess; one structural
              suspect is already identified and cheap: `chain_herd` runs EVERY cycle with
              `require_quality=True`, so the FINAL cycle is required to be continuable by a junction
              that will never run (`roll_candidates(require_quality=False)` is the documented
              terminal-roll mode and the chain never uses it). Cheap iteration exists now: dump a
              beam's input LOGS and rebuild it bit-exact on a fresh `FreeRun` (verified this session
              over all 7 cycle-2 nodes) instead of re-paying the ~475 s stage.
