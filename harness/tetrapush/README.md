...
| `away_walk.py` | **The AWAY-WALK escape atom** (session 65; Dereck's steer: the planner accounts for the reversal, herding is COMPLETE at separation, no A press, and an L that targets Tetra means the facing was wrong). `escape_atom` = slam-turn (a full stick within ±0x800 of the anti-travel-field fires `procMoveTurn(1)`: travel := stick, speed HALVES keeping sign -> **ground motion reverses up-herd in ONE acted frame and Tetra freezes the same frame at ~13.5 u of residual** -- one pipeline push, dead on the corridor) -> the L conversion (2 frames, the proc-7 DIR_BACKWARD negation, cone-checked at the ACT frame) -> release-L accel toward the exit bearing. `probe` sweeps the knobs and ranks L-cone compliance FIRST, then Dereck's s65 displacement bar (`WALK_FLOOR` 17 / `DIP_BUDGET` 1 post-separation frames under it). On the bed the compliant atom delivers Link **12.8 u from `ENTRY_ROLL_POS` at f17**, receding throughout, follow shell respected -- the handoff point where the Link-only 2D planners (`walk_to_entry` / `plan_land.reach_precise`) take over. MEASURED LIMITS (see the s65 status box): the fast L conversion is cone-blocked on the bed (the MoveTurn facing sweep crosses Tetra exactly on the negation frame), and NO model primitive reverses at >= 17 -- the compliant floor is ~12-13 sub-17 frames, vs Dereck's expected 0. Gated `tests/test_away_walk.py` (4). CLI `python -m harness.tetrapush.away_walk {probe\|trace}`. |
      ...
      - [~] **THE JUNCTION CANNOT FIX THE LATERAL EITHER -- STEERING AND ARMING ARE MUTUALLY
            ...
            - **NEXT (Dereck, end of session 64): WORK OUT THE AWAY-WALK. It is KNOWN TECH -- the
              ...
              **Session 65 worked out the away-walk's mechanics and shipped the atom -- see the box
              below.**
      - [~] **THE AWAY-WALK, WORKED OUT AND SHIPPED AS AN ATOM (session 65): the SLAM-TURN is the
            reversal, it freezes Tetra THE SAME FRAME, and the whole escape delivers Link to the
            entry region in ~17 frames -- but the model cannot meet Dereck's 0-dip bar, and that
            gap is the open question.** Dereck steered three times this session: (1) the placement
            planner must ACCOUNT for the optimal movement that reverses travel direction and heads
            roughly toward the roll-from region, with herding COMPLETE at separation, after which
            the Link-only leg borrows the existing 2D planners; (2) **the bar: true per-frame
            ground displacement below 17 u (the walk cap) for more than 1 frame = unoptimal,
            expected 0**; (3) **no A press in the turnaround, and an L that targets Tetra means
            the facing was toward her during the EBS** -- the L must act as a NO-ACTOR re-target.
            Developed on `synthetic_hot_arrival` (coord 287, feet 64) per the s64 handoff.
            - **THE ATOM (`away_walk.py`, gated `tests/test_away_walk.py`).** Slam-turn -> L
              conversion -> release-L accel; see the tooling row. The load-bearing measurement:
              `procMoveTurn(1)` (travel := the slam stick, `mNormalSpeed` halves KEEPING SIGN,
              6611/6623) **reverses ground motion in one acted frame**, which ends the contact
              push instantly -- **Tetra's residual is ONE pipeline push, 13.54 u, lat +0.21 (dead
              on the corridor)**, not the 27-82 u any brake-through escape leaves (a neutral coast
              plows her **+82 u** past the coord before freezing; a full-stick walk-back, +63 u
              and pd 16). So the terminal targeting should aim the LAST push so the slam frame's
              single residual push lands her ON the coord -- the undershoot is ~13.5 u, and it is
              deterministic.
            - **CONTACT displacement is recoil-halved, and that reframes the bar.** During the
              push Link's true displacement is ~12.9 u/f (foot term minus the full-depth recoil),
              i.e. herd frames are ALWAYS "sub-17" -- their currency is push. The s65 bar
              therefore counts from SEPARATION (`escape_atom` implements exactly that), which is
              also why the escape's slow frames should overlap contact where possible.
            - **NO MODEL PRIMITIVE REVERSES AT >= 17, measured exhaustively.** MOVE_TURN halves
              (-25.7 -> -12.9); the proc-7 negation flips travel AND speed so the heading is
              unchanged (s64's rule-3 correction -- it is a LAUNCH, not a reversal); `procSlip`
              skids to ZERO before flipping (6658: the flip fires at `|mNormalSpeed| <= 0.001`,
              re-seeded at max*0.5); speed targets are non-negative for a backslide (move.py
              95-99), so you cannot accelerate backward; walk accel is ~2.5 u/f^2; and beam
              searches over the full stick alphabet x L (depth 9/12/14, csangle swept, the
              pending-input dedup per the s64 warning) found **NO receding >= 17 state within 4
              post-separation dips**. The measured compliant floor on the bed is ~12-13 sub-17
              frames (the L-cone-clean atom rebuilds speed from ~0), ~5 if the L conversion could
              fire. **If Dereck's known tech really shows 0 sub-17 frames live, the model is
              missing a mechanic -- that is the next decomp target, and the away6/away7 scratch
              searches are the falsifiers to re-run.**
            - **THE L CONVERSION IS CONE-BLOCKED ON THE BED, and the geometry says why.** The
              negation needs TWO L frames (the first is the proc init -- the same init-frame law
              as proc 9, README s5 box), and the MoveTurn facing sweep (target = the slam stick =
              anti-travel-field = toward Tetra at the bed's terminal) crosses her +-0x4000 cone
              exactly on the negation frame -- so a compliant L (out-of-cone at the ACT frame,
              checked per Dereck's rule) never converts, and a held L ACQUIRES her (proc 9, the
              re-aim chases HER, ATN cap 12 -- shapes A/B measured it: receding at 12 u/f flat).
              On a real terminal whose travel field is oblique to her bearing the window may
              open; `escape_atom` reports `l_ok` per variant so the planner sees it.
            - **END TO END, the compliant atom on the bed:** freeze f1 (resid 13.54), motion
              reversed f2 (+12.9 up-herd), receding EVERY frame after, walking 17 from f13, **12.8
              u from `ENTRY_ROLL_POS` at f17**, follow shell respected (d_t max 226.8 < 230), no
              A, no lock. The entry leg then belongs to `walk_to_entry`/`reach_precise` (the
              borrowed 2D tools), which close the last ~13 u and set the entry facing 40835.
            - Offline suite **692 -> 696** (+`tests/test_away_walk.py` 4), 0 fail, same 8 xfails;
              land goldens byte-identical (no library change -- the harness only).
            - **NEXT, in order.** (1) **Settle the 0-dip question with Dereck** -- the model's
              reversal floor contradicts his expectation, so either name the missing mechanic
              (decomp-first: what does the live away-walk input stream look like?) or accept the
              ~5-frame crossing as herd-overlapped and move on. (2) **Wire rule 3 to the atom**:
              `objective.turnaround_ready` is still the s64-falsified predicate on disk; replace
              it with "the escape atom fires from the terminal state" (`escape_atom`'s l_ok +
              dips + receding, thresholds from the settled recipe), which re-scores the 73/74 f
              plans. (3) **Wire the terminal targeting to the residual**: aim the last push so
              the slam frame's 13.5 u lands her ON the coord (deterministic undershoot, no new
              search). (4) Then the entry leg: `walk_to_entry` from the atom's endpoint to
              `ENTRY_ROLL_POS`/facing 40835 -- the recorded solution's roll entry -- as the
              SEPARATE search s60 scoped.
            Do NOT re-pay: the ballistic L+A hops (sidehop/backflip reverse at 22.5-24.9 with
            zero dips but Dereck ruled A out for the turnaround, the direct ones TALK, and the
            post-turnaround ones ramp slowly + break the follow shell), the junction moves (s64),
            terminal ranks (s63), the cycle count (s63), or "more push" (98.5% saturated).
